...
func (u *Updater) StartBackground(ctx context.Context) {
	...
	u.bgStarted = true
	u.mu.Unlock()
	u.logger.Printf("update: enabled, channel=%s, interval=%s", cfg.Channel, cfg.CheckInterval)
	go u.loop(ctx)
}
...
func (u *Updater) download(ctx context.Context, release *releaseInfo) (string, error) {
	...
	if sha256Asset == nil {
		return "", fmt.Errorf("no sha256 asset found for %s in release %s", targetName, release.TagName)
	}
	...
	finalName := "easy-proxies-" + sanitizePathPart(release.TagName)
